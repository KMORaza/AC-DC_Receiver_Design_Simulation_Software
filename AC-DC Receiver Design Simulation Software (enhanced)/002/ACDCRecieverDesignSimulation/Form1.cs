        ...
        private async void btnSimulate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (parameters.Regulator == RegulatorType.Switching)
                {
                    ...
                    {
                        thermalRunaway = switchDevice.ThermalModel.ThermalRunawayDetected;
                    }
                }
                ...
                lblDeviceTemperature.Text = $"{result.DeviceTemperature:F1} °C";
                lblDeviceEfficiency.Text = $"{result.DeviceEfficiency:F1} %";
                lblThermalRunaway.Text = thermalRunaway ? "Warning: Thermal Runaway Detected" : "Safe";
                ...
                {
                    if (result.BodeFrequencyPoints[i] > 0)
                    {
                ...
                lblStatus.Text = thermalRunaway ? "Simulation Complete (Thermal Warning)" : "Simulation Complete";
                lblStatus.ForeColor = thermalRunaway ? Color.Orange : Color.LimeGreen;
            }
            catch (Exception ex)
            {
